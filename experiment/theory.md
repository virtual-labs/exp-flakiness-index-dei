...
Mean Dimension = (Size of sieve through which particle passes + Size of sieve on which particle is retained) ÷ 2

Gauge Thickness (Critical Thickness)

Gauge Thickness = 0.6 × Mean Dimension

Individual Fraction Flakiness Index

Flakiness Index for Fraction = (yᵢ / Yᵢ) × 100

Where:
- yᵢ = Weight of flaky material passing gauge for that fraction (g)
- Yᵢ = Total weight of that fraction tested (g)

Overall Flakiness Index

Flakiness Index (FI) = (B / A) × 100

Where:
- A = Total weight of sample taken = Y₁ + Y₂ + Y₃ + ... + Yₙ (g)
- B = Total weight of material passing through various slots = y₁ + y₂ + y₃ + ... + yₙ (g)

Alternative Expression

Flakiness Index (%) = [(y₁ + y₂ + y₃ + ... + yₙ) / (Y₁ + Y₂ + Y₃ + ... + Yₙ)] × 100